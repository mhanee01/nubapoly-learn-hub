...
-- Drop policies that depend on is_lecturer_or_admin function
DROP POLICY IF EXISTS "Lecturers and admins can view all profiles" ON public.profiles;

-- Drop all functions that depend on user_role
DROP FUNCTION IF EXISTS is_lecturer_or_admin(uuid) CASCADE;
...
-- Create new simplified policy for admins to view all profiles
